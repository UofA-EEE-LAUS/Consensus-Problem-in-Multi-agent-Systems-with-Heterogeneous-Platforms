...
clientID=vrep.simxStart('127.0.0.1',19999,true,true,5000,5);
 ...
 [returnCode,Quadricopter_propeller_respondable1]=vrep.simxGetObjectHandle(clientID,'Quadricopter_propeller_respondable1',vrep.simx_opmode_oneshot_wait);
if (clientID>-1)
    disp('connected');
    
    switch_move=input('Whether to perform displacement simlation input 1(linear motion) or 0(circlar motion) or 3(for landing)');
   ...
   % [returnCode]=vrep.simxSetObjectPosition(clientID,Quadricopter_target,-1,[1,1,1],vrep.simx_opmode_blocking);
    ...
    elseif switch_move==3
       ...
       pause(0.4);
        ...
        landFlag = 1;
        packedData = [x y z landFlag];
        PackedD = vrep.simxPackFloats(packedData);
       [returnCode]=vrep.simxWriteStringStream(clientID,'stringname',PackedD,vrep.simx_opmode_oneshot);
        pause(2)
